...
<body>
	...
	<% if(request.getSession().getAttribute("treino") == null){
		...
		out.print("<a class=\"btn btn-primary btn-lg\" href=\"./addWod.jsp\" role=\"button\">Cadastrar Wod</a>");
		out.print("</div></div>");
	} else{
		Treino treino = (Treino) session.getAttribute("treino");
		String tempURLu = request.getContextPath() + "/ServletTreino?acao=update&treinoID=" + treino.getId();
		String tempURLd = request.getContextPath() + "/ServletTreino?acao=delete&treinoID=" + treino.getId();
		...
		out.print("<label for=\"inputTitle\">Titulo:</label> <input type=\"text\" class=\"form-control\" id=\"inputTitle\" placeholder=\"Titulo\" name=\"title\" maxlength=\"10\" value="+ treino.getTitulo()+"></div></div>");
		out.print("<div class=\"form-row\"><div class=\"form-group col-sm-12\"><label for=\"inputDescription\">Descrição:</label> <textarea class=\"form-control\" id=\"inputDescription\" placeholder=\"Descrição\" name=\"description\" rows=\"10\" cols=\"20\">"+ treino.getDescricao()+"</textarea>");
		out.print("</div></div><input type=\"submit\" class=\"btn btn-warning\" value=\"editar\" formaction="+ tempURLu +"><input type=\"submit\" class=\"btn btn-danger\" value=\"deletar\" formaction="+ tempURLd +"></form></div></div></div></div></div>");
	}
		%>
